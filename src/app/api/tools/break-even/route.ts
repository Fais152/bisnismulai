...
export async function POST(request: Request) {
  try {
    ...
    if (existing) {
      ...
      if (error) {
        console.error("Break-even update error:", error);
    ...
    if (error) {
      console.error("Break-even insert error:", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ message: "Created", data: created }, { status: 201 });

  } catch (error: any) {
    console.error("API Tools Break-even Error:", error);
...
export async function GET() {
  try {
    ...
    if (error && error.code !== "PGRST116") {
      console.error("Break-even GET error:", error);
    }

    return NextResponse.json({ data: data?.data ?? null });
  } catch (error: any) {
    console.error("Break-even GET exception:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
